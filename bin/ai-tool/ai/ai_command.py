...
def generate_command(user_command, os):
    ...
    message = client.messages.create(
        ...
        messages=[
            {
                "role": "system",
                "content": f"""You will generate a {os} command a terminal understands based on the user command given and the data available, you will translate their command essentially. For example, "create a folder named rodeo"
                ...
                "create a new folder called terminal" and all the data available is "5:00 : mkdir terminal, mkdir: cannot create directory ‘terminal’: File exists'", You will assess the situation and return a new command like "dir" to check what is in the directory which will store
